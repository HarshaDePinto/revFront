...
import { updateStatus, updateDelivery } from "./apiAcceptedOrders";
...
function SingleOrder({ singleOD, setShowAll, setShowSingle }) {
  ...
  const onClickDelivered = (order) => {
    ...
    let cashierId = user._id;
    let cashierName = user.name;

    updateDelivery(order._id, {
      status,
      paymentStatus,
      cashierId,
      cashierName,
      ...
      <div className="recipt-sec padding-20">
        ...
        <div style={{ display: "none" }}>
          <div ref={componentRef}>
            <PrintInvoice
              clientName={singleOD.clientName}
              orderId={singleOD.orderId}
              products={singleOD.products}
              paymentMethod={singleOD.paymentMethod}
              paymentStatus={singleOD.paymentStatus}
              totalAmount={singleOD.totalAmount}
              discount={singleOD.discount}
              finalAmount={singleOD.finalAmount}
            />
          </div>
        </div>
        <div style={{ display: "none" }}>
