...
function CreateNew({ setMainValues, values }) {
  ...
  useEffect(() => {
    setFoodValues((v) => ({ ...v, formData: new FormData() }));
  }, []);
  ...
  const handleChange = (name) => (event) => {
    ...
    setFoodValues({ ...foodValues, [name]: value });
  };

  const clickSubmit = (e) => {
    e.preventDefault();
    setFoodValues({ ...foodValues, error: "", loading: true });
    createFood(user._id, token, formData).then((data) => {
      if (data.error) {
        setFoodValues({ ...foodValues, error: data.error, loading: false });
      } else {
        setMainValues({
          ...values,
          ...
          foodCategoryId: data.foodCategory,
        });
      }
    });
  };
  ...
  return (
    <Fragment>
      <h5 className="text-light-black fw-700">Create New Food</h5>
      {showLoading()}
      {showError()}
      {newSlideForm()}
    </Fragment>
