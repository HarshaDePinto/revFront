...
import { getFoodByCategory } from "./apiFood";
...
function SingleCategory({
  ...
  const showCount = () => (
    <div
      ...
      style={{ display: foodCount ? "" : "none" }}
    >
      <h2>No Foods Under This Category!</h2>
    </div>
  );

  const viewSingleFood = (id) => {
    setMainValues({
      ...values,
  ...
  const getImage = (s) => {
    return (
      <img
        src={`${API}/Food/photo/${s}?${imageHash}`}
  ...
  return (
    <Fragment>
      <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12">
        <h5 className="text-light-black fw-700">{foodCategoryName}</h5>
      </div>
      {showLoading()}
      {showCount()}
      {showError()}
