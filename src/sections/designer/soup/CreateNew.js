...
import { createSoup } from "./apiSoup";
import { Roller } from "react-awesome-spinners";

function CreateNew({ setMainValues, values }) {
  const { user, token } = isAuthenticated();
  const [soupValues, setSoupValues] = useState({
    title: "",
    subtitle: "",
    description: "",
    price_junior: "",
  ...
  useEffect(() => {
    setSoupValues((v) => ({ ...v, formData: new FormData() }));
  }, []);
  ...
  const handleChange = (name) => (event) => {
    ...
    setSoupValues({ ...soupValues, [name]: value });
  };

  const clickSubmit = (e) => {
    e.preventDefault();
    setSoupValues({ ...soupValues, error: "", loading: true });
    createSoup(user._id, token, formData).then((data) => {
      if (data.error) {
        setSoupValues({ ...soupValues, error: data.error, loading: false });
      } else {
        setMainValues({
          ...values,
  ...
  return (
    <Fragment>
      <h5 className="text-light-black fw-700">Create New Soup</h5>
      {showLoading()}
      {showError()}
      {newSlideForm()}
    </Fragment>
