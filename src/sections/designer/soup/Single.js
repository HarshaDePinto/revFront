...
import { readSoup, updateSoup, deleteSoup } from "./apiSoup";
import { Roller } from "react-awesome-spinners";

function Single({ setMainValues, values, soupId }) {
  ...
  const clickSubmit = (e) => {
    ...
    updateSoup(token, soupId, user._id, formData).then((data) => {
      if (data.error) {
  ...
  const deleteAddOnClick = (e) => {
    ...
    deleteSoup(token, soupId, user._id, formData).then((data) => {
      if (data.error) {
