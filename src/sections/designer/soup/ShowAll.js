...
import { getAllSoups } from "./apiSoup";
...
function ShowAll({ setMainValues, values }) {
  ...
  const [soupCount, setSoupCount] = useState(false);
  ...
  const showCount = () => (
    <div
      ...
      style={{ display: soupCount ? "" : "none" }}
    >
      <h2>No Soups Added Yet!</h2>
    </div>
  );

  const viewSingleSoup = (id) => {
    setMainValues({
      ...values,
      showAllSoup: false,
      showSingleSoup: true,
  ...
  const getImage = (s) => {
    return (
      <img
        src={`${API}/soup/photo/${s}?${imageHash}`}
  ...
  return (
    <Fragment>
      <div className="col-xl-12 col-lg-12 col-md-12 col-sm-12">
        <h5 className="text-light-black fw-700">All Soups</h5>
      </div>
      {showLoading()}
      {showCount()}
      {showError()}
