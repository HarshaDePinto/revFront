...
function Single({
  ...
  useEffect(() => {
    ...
    readFoodCategory(singleId,abortCont).then((data) => {
     if (data){
      if (data.error) {
  ...
  const clickSubmit = (e) => {
    ...
    updateFoodCategory(token, singleId, user._id, formData).then((data) => {
      if (data.error) {
        setValues({ ...values, error: data.error });
      } else {
        if (showUpdateFoodCategory) {
          setShowUpdateFoodCategory(false);
        } else {
          setShowUpdateFoodCategory(true);
        }
  ...
  const editAddOnForm = () => {
    return (
      <form>
        <h5 className="text-light-black fw-700">Update Food Category </h5>
        {showLoading()}
        {showSuccess()}
        {showError()}
        {!loading && (
  ...
  const deleteAddOnClick = (e) => {
    e.preventDefault();
    setValues({ ...values, error: "", loading: true });
    deleteFoodCategory(token, singleId, user._id, formData).then((data) => {
      if (data.error) {
        setValues({ ...values, error: data.error });
