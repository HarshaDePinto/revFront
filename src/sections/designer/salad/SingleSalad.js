...
import { readSalad, updateSalad, deleteSalad } from "./apiSalad";
import CheckBox from "./Checkbox";
import { Roller } from "react-awesome-spinners";

function SingleSalad({ setMainValues, values, saladId, categoryId }) {
      ...
      if (data) {
        if (data.error) {
          ...
          setCreateValues((v) => ({
            ...
            saladCategory: data.saladCategory,
  ...
  const clickSubmit = (e) => {
    ...
    updateSalad(token, saladId, user._id, formData).then((data) => {
      if (data.error) {
  ...
  const editAddOnForm = () => {
    return (
      <div className="col-12">
        <form>
          <h5 className="text-light-black fw-700">Update Salad </h5>
          {showLoading()}
          {showSuccess()}
          {showError()}
          {!loading && (
  ...
  const deleteAddOnClick = (e) => {
    ...
    deleteSalad(token, saladId, user._id, formData).then((data) => {
      if (data.error) {
